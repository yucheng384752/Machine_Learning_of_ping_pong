...
def main():
    ...
    #  載入你用 env_paia 訓練的模型
    model = load_trained_model(env, device, "models\\dqn_pong_last.pt")
        ...
        if env.use_obstacle:
            pygame.draw.rect(
                ...
                (
                    env.obstacle_x,
                    env.obstacle_y,
                    env.OBSTACLE_W,   # 用環境裡定義的寬度
                    env.OBSTACLE_H,   # 用環境裡定義的高度
                ),
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--opponent",
        type=str,
        default="predictive-normal",
        help="2P 對手類型，例如 simple-easy/simple-normal/simple-fast/predictive-normal/predictive-hard",
    )
    parser.add_argument(
        "--mode",
        type=str,
        default="normal",
        help="環境模式：easy/normal/hard（會影響障礙物等）",
    )
